...
from NNFP import normalize_array, tictoc
from NNFP import Deep_neural_network
from NNFP import Finger_print
from NNFP import mol_graph
from NNFP import graph_from_smiles_tuple, degrees, graph_from_smiles
...
def array_rep_from_smiles(smiles):
    ...
    #print len(molgraph.rdkit_ix_array())
...
def main():
	...
	x_trains = [array_rep_from_smiles(x) for x in x_trains]
	#x_trains = array_rep_from_smiles(tuple(x_trains[0:1]))
	print len(x_trains)
	y_trains = np.reshape(y_trains, (N_train, 1))
	x_vals = np.reshape(x_vals, (N_val, 1))
	x_vals = array_rep_from_smiles(x_vals)
	y_vals = np.reshape(y_vals, (N_val, 1))
	x_tests = np.reshape(x_tests, (N_test, 1))
	x_tests = array_rep_from_smiles(x_tests)
