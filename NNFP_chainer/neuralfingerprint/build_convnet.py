...
def sum_and_stack(features, idxs_list_of_lists):
    #print "features"
    return fast_array_from_list([np.sum(features[idx_list]._data[0], axis=0) for idx_list in idxs_list_of_lists])
...
def matmult_neighbors(self, array_rep, atom_features, bond_features, get_weights_func):
    ...
    for degree in degrees:
        ...
        if len(atom_neighbors_list) > 0:
            ...
            np_activations = (activations._data[0])
            activations_by_degree.append(np_activations)
