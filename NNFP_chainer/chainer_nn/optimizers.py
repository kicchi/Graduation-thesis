...
def rms_prop(grad, x, callback=None, num_iters=100, step_size=0.1, hammma=0.9, eps = 10**-8):
	"""Root mean squared prop: See Adagrad paper for details."""
	avg_sq_grad = np.ones(len(x)) #Is this really a sensible initialization?
	for i in xrange(num_iters):
		g = grad(x, i)
		if callback: callback(x, i)
		velocity = mass * gamma + g**2 * (1 - gamma)
...
def adam(grad, x, callback=None, num_iters=100, step_size=0.001, b1=0.9, b2=0.999, eps=10**-8):
	"""It's basically TMSprop with momentum and some correction terms."""
	m = np.zeros(len(x))
	v = np.zeros(len(x))
	for i in range(num_iters):
		...
		v = (1 - b2) * (g**2) + b2 * v #Second moment estimete
		mhat = m / (1 - b1**(i + 1)) #Bias correction
		vhat = v / (1 - b2**(i + 1))
		x -= step_size * mhat / (np.sqrt(vhat) + eps)
	print "adama return ", len(x)
	return x
...
def bfgs(obj_and_grad, x, callback=None, num_iters=100):
	...
	res = minimize(fun=obj_and_grad, x0=x, jac=True, callback=wrapped_callback, options = {'maxiter':num_iters, 'disp':True})
	return res.x
