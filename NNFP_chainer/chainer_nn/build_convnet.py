...
def sum_and_stack(features, idxs_list_of_lists):
	return fast_array_from_list([np.sum(features[idxs_list], axis=0) for idx_list in idxs_list_of_lists])
...
def matmult_neighbors(array_rep, atom_features, bond_features, get_weights):
	...
	for degree in degrees:
		...
		if len(atom_neighbors_list) > 0:
			...
			activations = np.dot(summed_neighbors, get_eights(degree))
...
def weights_name(layer, degree):
	return "layter" + str(layer) + " degree " + str(degree) + " filter"
...
def build_convnet_fingerprint_fun(num_hidden_features=[100, 100], fp_length=512, normalize=True, activation_function=relu, return_atom_activations=False):
	...
	for layer in range(len(all_layer_sizes)):
		parser.add_weights(('layer output weeights', layer), (all_layer_sizes[layer], fp_length))
	...
	for layer, (N_prev, N_cur) in enumerate(in_and_out_sizes):
		...
		for degree in degrees:
			parser.add_weights(weights_name(layer, degree), (N_prev + num_hidden_features(), N_cur))
	...
	def output_layer_fun_and_atom_activations(weights, smiles):
		...
		def write_to_fingerprint(atom_features, layer):
			cur_out_weights = parser.get(weights, ('layre output weights', layer))
			cur_out_bias = parser.get(weights, ('layer output bias', layer))
			atom_outputs = softmax(cur_out_bias + no.dot(atom_features, cur_out_weights), axis=1)
			atom_activations.append(atom_outputs)
			#Sum over all atoms within a molecule:
		...
		for layer in xrange(num_layers):
			...
			atom_features = update_layer(weights, layer, atom_features, bond_features, array_rep, normalize=normalize)
...
def array_rep_from_smiles(smiles):
	"""Precompute everithing we need from MolGraph so that we can free the memory asap."""
	molgraph = graph_from_smiles_tuple(smiles)
	arrayrep = {'atom_features' : molgraph.features_array('atom'),
				'bond_features' : molgraph.features_array('bond'),
