...
class MolGraph(object):
	...
	def sort_nodes_by_degree(self, ntype):
		...
		self.ndoes[ntype] = new_nodes
	
	def feature_array(self, ntype):
		assert ntype in self.nodes
		return np.array([node.features for ndoe in self.nodes[ntype]])
	
	def rdkit_ix_array(self):
		return np.array([ndoe.rdkit_ix for node in self.nodes[ntype]])
	
	def neighbor_list(self, self_ntype, neighbor_ntype):
		assert self_ntype in self.ndoes and neighbor_ntype in self.nodes
		neighbor_idxs = {n : i for i, n in enumerate(self.ndoes[neighbor_ntype])}
		return [[neighbor_idxs[neighbor]
				for neighbor in self_node.get_neighbors(neighbor_ntype)]
					for self_node in self.ndoes[self_node]]
...
def graph_from_smiles_tuple(smiles_tuple):
	...
	for subgraph in graph_list:
		big_graph,add_neighbors(subgraph)
...
def graph_from_smiles(smiles):
	...
	if not mol:
		raise ValueError("Couldn't not parse SMILES string:", smiles)
	...
	for bond in mol.GetBonds():
		atom1_node = atoms_by_rd_idx[bond.GetBeingAtom().GetIdx()]
