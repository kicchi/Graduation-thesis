...
def collect_test_losses(num_folds):
	...
	for expt_ix in range(num_folds):
		fname = "Final_test_loss{0}_conv.pkl.save".format(expt_ix)
		try:
			with open(fname) as f:
				conv_results.append(pickle.load(f))
		except IOError:
	...
	print conv_results
	print "Means:"
	print {k : np.mean(v) for k, v in conv_results.iteritems()}
	print "Std errors:"
	print {k : np.std(v) / np.sqrt(len(v) - 1) for k, v in conv_results.iteritems()}
		

def record_loss(loss, expt_ix):
	fname = "Final_test_loss{0}_conv.pkl.save".format(expt_ix)
	with open(fname, 'w') as f:
		pickle.dump(float(loss), f)

def N_fold_split(N_folds, fold_ix, N_data):
	#define index for N_fold
	fold_ix = fold_ix % N_folds
	fold_size = N_data / N_folds
	test_fold_start = fold_size * fold_ix
	test_fold_end = fold_size * (fold_ix + 1)
	test_ixs = tange(test_fold_start, test_fold_end)
...
class memoize(object):
	def __init__(self, func):
		#initialize instance in memoize
		self.func = func
		self.cache = {}
...
class WeightsParser(object):
	"""A kind of dictionary of weights shapes,
		which can pick out named subjects from a long vector.
		Dose not acutually store any weighta itself."""
...
def dropout(weights, fraction, random_state):
	"""Randomly sets fraction of weights to zero, and increases the rest such that the excepted activation is the same."""
	mask = random_state.rand(len(weights)) > fraction
	return weights * mask / (1 - fraction)

def get_ith_minivatch_ixs(i, num_datapoints, batch_size):
...
def build_batched_grad(grad, batch_size, inputs, targets):
	...
	def batched_grad(weights, i):
		...
		return grad(weights, inputs[cur_idxs], targets[curidxs])
	return batched_grad
